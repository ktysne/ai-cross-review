...
Codex desktop で提示するときは、利用可能なツール一覧を確認し、短い選択肢をクリック可能な UI として表示するユーザ入力ツールを必ず使う。
...
現在の実行モードでユーザ入力ツールを利用できない場合は、本文の 3 択へ自動的に切り替えない。
「このモードでは選択 UI を使用できない」と説明し、選択 UI を利用できるモードまたはセッションで再提示する必要があることを伝える。
並び順は固定です。
...
- PR コメントは手で組み立てず、裏取りと対応を `.cross-review/round-<N>-triage.md` に書いてから `node tools/cross-review.js comment --round <N> [--verify <検証出力>]` で本文を生成し、`gh pr comment <番号> --body-file <生成物>` で投稿する (レビュー出力は往復ごとに `.cross-review/` へ自動保存される。`subagent` 経路で出力を `round-<N>-subagent.md` へ貼るのは任意で、記録用。`comment` は読みません。詳細は `docs/cross-review.md`)。
