...
- `comment` は開始時にブランチ名を取得できない場合、平置きへ戻らず失敗する。既定出力先の既存コメント本文は入力検査前に削除するが、`--out` で明示した出力先は新しい本文を書き出すまで変更しない。メタ情報、判断ファイル、検証出力、本文保存、投稿のいずれかが失敗した場合は今回生成した本文を残さない。判断ファイルが無い場合は雛形だけを書き出して終了コード 1 で終わる。
