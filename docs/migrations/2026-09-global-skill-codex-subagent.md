...
## 取り込み先で必要な作業
- `~/.codex-subagent/skills/` がある環境では、`node tools/cross-review.sync-all.js --global-skill`(このリポジトリなら `npm run sync:global`。sync-all を入れていない取り込み先では上流の checkout で実行する)を一度実行し、サブエージェント側にも SKILL を配る。無い環境では何もしなくてよい。
