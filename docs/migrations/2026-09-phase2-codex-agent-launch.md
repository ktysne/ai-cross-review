...
- bridge 経由には 2 つの前提があり、起動前に検証します。スクリプトが `approval_policy=never` を明示していなければ警告を出して直接起動へ戻し（承認 never 固定を保つため）、定義ファイルの `codex_sandbox` が `--fix` の有無と食い違えば起動せずエラー（終了コード 2）で止めます。
