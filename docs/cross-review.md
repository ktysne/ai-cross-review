...
- **対象外（skip）**: `cross-review.sync.json` という名前でも、内容が `upstream` / `files` を持たないもの（旧 `{source, ref, commit}` 形式の独自 sync 来歴や、別用途でたまたま同名のファイル）は **同期対象外（対象外）** として skip し、エラーにはしません。`/Develop` に新旧の流儀が混在していても全体を失敗させないためです。
