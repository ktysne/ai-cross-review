...
選択肢 2 も同じく実装者で決まります。  
Claude 主導なら `node tools/cross-review.js subagent` の出力を Claude の客観サブエージェントへ渡し、Codex 主導なら `npm run review:codex` を実行します。
...
汎用観点（`GENERIC_CHECKLIST`）と `.cross-review.example.md` には、観点そのものに加えて「指摘の出し方」を 4 項目入れてあります（分類と重大度順、到達しない値を前提にした指摘を出さない、blocker と要修正は合計 10 件まで、各指摘に反例を一行）。  
