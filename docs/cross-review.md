...
指定した経路が codex 経路で、利用上限などにより使えないときは、CLI のフォールバック（終了コード 75）に従って subagent（Claude の客観サブエージェント）で回し、切り替えたことを報告に書きます。Claude 側の経路にはこのフォールバックが無いので、使えなければ報告に書いて判断を仰ぎます（自走中は判断待ちの事項として載せます）。
