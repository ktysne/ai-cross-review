...
`comment` は入力検査の前に既定出力先の古い本文を削除します。`--out` で明示した出力先は、入力検査に成功して新しい本文を書き出すまで変更しません。メタ情報が無い、複数ある、検証出力を読めない、判断ファイルが無い、本文を書けない、投稿に失敗するといった場合は終了コード 1 で終わり、今回生成した本文を残しません。`--post` を付けた場合は、本文を出力先へ保存してから同じメモリ本文を投稿し、投稿に失敗すると保存した本文を削除します。投稿ではネットワーク遅延を考慮して gh のタイムアウトを 60 秒に延長し、タイムアウトや起動エラーの理由を表示します。
