...
> **このドキュメントは汎用の相互レビュー「フロー doc」です。** 他リポジトリへは**そのまま（verbatim）取り込み**でき、
> 取り込み先では**編集しない**（再同期で上書きする）。プロジェクト固有の運用・観点は取り込み側の別ファイルに書く
> （レビュー観点は `.cross-review.md`、アプリ固有メモは取り込み側の overlay doc）。リポ内ファイルは可搬性のため
> リンクでなくインラインコードで示し、節参照は本文中の名前で行う（取り込み先の階層やアンカー生成方式に依存しないため）。
...
| Agent ツールの客観サブエージェント | リモートコントロール環境で Codex の代わりに客観レビューを担う（同セッション内・後述「リモートコントロール環境でのフォールバック」節） |
...
（チャット本文だけに残さない＝揮発させない。詳細は後述「PR を共有ログにする」節）。
...
- 申し送りファイルは**リポジトリ外か `.gitignore` 済みパス**に置くのが無難（`--uncommitted` の untracked
  ...
  レビュー差分から自動除外される（`tools/cross-review.js` の `collectReviewDiff`）。
...
観点を増減したら `.cross-review.md` を更新する。新規プロジェクトへ導入する場合は `.cross-review.example.md` を雛形にする。
...
- 運用ルールの要約は取り込み側の `CLAUDE.md` / `AGENTS.md`（あれば）。このドキュメントが正本で内容を二重化しない（要約からはここへリンクする）。
