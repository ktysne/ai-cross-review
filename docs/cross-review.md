...
| Agent ツールの客観サブエージェント | CLI を起動できない環境で、Codex の代わりにレビューする（同じセッション内。後述「CLI を起動できない環境での代替」節） |
...
> Claude から `npm run review:codex*` を実行するときは、**codex がネットワークを使う**ため、サンドボックス内では接続に失敗することがあります。  
> ネットワークを許可して実行してください。

## CLI を起動できない環境での代替（Codex の代わりに客観サブエージェント）

Claude を**クラウドで実行**していると、手元の `codex` / `claude` CLI を起動できないことがあります（PATH に無い・起動できない）。  
このときはレビュアーを **Codex ではなく「Claude の客観的なサブエージェント」**に切り替えます。

**見分け方**: `codex` が PATH で見つからない（`Get-Command codex` / `which codex` が失敗する）か、クラウド実行だと分かっているとき。  
`npm run review:codex` が CLI 不在で失敗したときも、この代替に切り替えます。

**仕組み**: プロンプトを組み立てる部分（観点の解決・差分の収集・モード別の指示付け）は **node と git だけ**で動くので、クラウド環境でもそのまま使えます。  
外部 CLI を起動する部分だけを「Agent ツールのサブエージェント」に置き換えます。  
...
3. サブエージェントのレビュー結果（または修正差分）を Claude が読み、A / B と同じく先（修正の適用・妥当性確認）へ進める。  
   妥当性確認も同じく `subagent` のプロンプトとサブエージェントで回す。  
   往復は最大 3 回までの数え方も変わらない。

**起点 3 択（A / B / C）との対応**: クラウドでも 3 択の意味は変わらず、レビュアーだけが Codex → 客観サブエージェントに替わります。  
A → `subagent`（修正なし）を読み取り系へ、B → `subagent --fix` を書き込み権限ありへ、C → そのまま。

> 同じ Claude が実装とレビューを兼ねても、サブエージェントは**別の文脈**で差分だけを見るので、「客観的な第三者レビュー」の利点（実装時の思い込みに引きずられない）は保てます。  
> **`codex` CLI が使える環境では、これまでどおり Codex を優先します**（別のモデルの視点が得られるため。この代替は、クラウド実行などで Codex を起動できないときの代わりです）。
